...
//     console_printf("ts:%u theta:%.4f accL:[%.3f %.3f %.3f] accR:[%.3f %.3f %.3f] accCG:[%.3f %.3f %.3f] gyr:[%.3f %.3f %.3f] magL:[%.3f %.3f %.3f] magR:[%.3f %.3f %.3f] magCG:[%.3f %.3f %.3f]\n",
//                    data.timestamp_ms,
//                    data.hinge_theta,
...
// static void hinge_update(float dt, const imu_data_t* imuL, const imu_data_t* imuR, const mag_data_t* magL, const mag_data_t* magR, hinge_state_t* h)
// {
//     const float hinge_axis[3] = { 1.0f, 0.0f, 0.0f };

//     /* Gyro-based prediction: hinge rate is half the differential around hinge axis */
//     float wL = imuL->gyr_B_radDs[0];
//     float wR = imuR->gyr_B_radDs[0];
//     float theta_dot_meas = 0.5f * (wR - wL) - h->theta_bias;
//     float theta_pred = h->theta + dt * theta_dot_meas;

//     /* Acc-based observation */
...
//         theta_obs = hinge_obs_from_vecs(hinge_axis, gL, gR);
//     } else if (magL && magR) {
...
//         vec3_normalize(mL);
//         vec3_normalize(mR);
//         theta_obs = hinge_obs_from_vecs(hinge_axis, mL, mR);
//     }

//     float err = theta_obs - theta_pred;
//     h->theta = theta_pred + DUAL_IMU_ANGLE_KP * err;
//     h->theta_bias += DUAL_IMU_ANGLE_KI * err * dt;
// }

// static void att_compute_body_vectors(float theta, const imu_data_t* imuL, const imu_data_t* imuR, const mag_data_t* magL, const mag_data_t* magR, dual_imu_att_output_t* out)
// {
...
//     /* Remove lever-arm centripetal terms then average to approximate CG specific force */
//     float acc_cg_L[3];
//     float acc_cg_R[3];
//     float cross_tmp[3];
//     float omega_cross_r[3];
//     vec3_cross(gyr_b_avg, pos_b_L, omega_cross_r);
//     vec3_cross(gyr_b_avg, omega_cross_r, cross_tmp);
//     acc_cg_L[0] = acc_b_L[0] - cross_tmp[0];
//     acc_cg_L[1] = acc_b_L[1] - cross_tmp[1];
//     acc_cg_L[2] = acc_b_L[2] - cross_tmp[2];
//     vec3_cross(gyr_b_avg, pos_b_R, omega_cross_r);
//     vec3_cross(gyr_b_avg, omega_cross_r, cross_tmp);
//     acc_cg_R[0] = acc_b_R[0] - cross_tmp[0];
//     acc_cg_R[1] = acc_b_R[1] - cross_tmp[1];
//     acc_cg_R[2] = acc_b_R[2] - cross_tmp[2];
...
//             hinge_update(dt, &g_state.imu_local, &g_state.imu_remote, &g_state.mag_local, &g_state.mag_remote, &g_state.hinge);
//             att_compute_body_vectors(g_state.hinge.theta,
//                                      &g_state.imu_local,
//                                      &g_state.imu_remote,
//                                      &g_state.mag_local,
//                                      &g_state.mag_remote,
